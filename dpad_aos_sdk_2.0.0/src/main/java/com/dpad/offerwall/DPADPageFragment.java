...
//                ToastManager.showShort(getActivity(), "적립 완료되었습니다.");
    ...
    private boolean containType(List<DPAdTabInfo> tabInfos,String type){
        for(int i=0;i<tabInfos.size();i++){
           if( tabInfos.get(i).type_list.contains(type)){
               return  true ;
           }
    ...
    private void setupNewData(){
        DPAdTabInfo dpAdTabInfo =  getArguments().getParcelable("data");
//                DPAdInfo dpAdInfo_t = new DPAdInfo();
//                dpAdInfo_t.setRwd("30P");
//                adapter.getmDatas().add(0,dpAdInfo_t);
        DPADOfferwallFragment parentFrag=   ((DPADOfferwallFragment)getParentFragment());
        if(parentFrag!=null) {
            if (dpAdTabInfo.type_list == null) {
                adapter.setmDatas(parentFrag.itemList);
            } else {
                List<DPAdInfo> newList = new ArrayList<>();
                if(dpAdTabInfo.type_list.isEmpty()){
                    for (int i = 0; i < parentFrag.itemList.size(); i++) {
                        if(!containType(parentFrag.tabList,parentFrag.itemList.get(i).getRevenue_type())){
                            newList.add(parentFrag.itemList.get(i));
                        }
                    }
                }else{
                    for (int i = 0; i < parentFrag.itemList.size(); i++) {
                        if(dpAdTabInfo.type_list.contains(parentFrag.itemList.get(i).getRevenue_type())){
                            newList.add(parentFrag.itemList.get(i));
                        }
                    }
                }
    ...
    private void CallAdList(Boolean fromSwipe ) {
        if (fromSwipe) {
               DPAD.getAdList(getActivity(), new AdListCallBack() {
                ...
                public void onSuccess(List<DPAdInfo> dpads) {
                    if(getParentFragment()!=null){
                        DPADOfferwallFragment parentFrag=   ((DPADOfferwallFragment)getParentFragment());
                        parentFrag.itemList.clear();
                        parentFrag.itemList.addAll(dpads);
                    }
                    setupNewData();
                }
            ...
            if(getParentFragment()!=null) {
                DPADOfferwallFragment parentFrag = ((DPADOfferwallFragment) getParentFragment());
                if(parentFrag.itemList.isEmpty()){
                    CallAdList(true);
                    return;
                }
            }
